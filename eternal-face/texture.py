...
class Texture():
    ...
    def __init__(self, source, dims, window, correspondence):
        self.source = source
        self.dims = dims
        self.w = window
        self.k = (window - 1)/2
        self.overlap = window/6
        self.adj_wind = self.w - self.overlap
        self.min_err = 0.05
        ...
        pad_row = (dims[0] / self.adj_wind + 1) * self.adj_wind + self.overlap - dims[0]
        pad_col = (dims[1] / self.adj_wind + 1) * self.adj_wind + self.overlap - dims[1]
        ...
        if self.correspondence is not None:
            self.correspondence = cv2.copyMakeBorder(cv2.cvtColor(correspondence, cv2.COLOR_BGR2GRAY),
                                                     0, pad_row, 0, pad_col, cv2.BORDER_REFLECT_101)
    ...
    def generate_candidates(self):
        ...
        upper_row = self.source.shape[0] - self.k - 1
        upper_col = self.source.shape[1] - self.k - 1
        if upper_row == self.k:
            upper_row += 1
        if upper_col == self.k:
            upper_col += 1
        ...
        self.candidates = np.zeros(((upper_row - self.k) * (upper_col - self.k),
                                    self.w, self.w, 3))
        ...
        for row in range(self.k, upper_row):
            for col in range(self.k, upper_col):
                patch = self.source[row-self.k:row+self.k+1, col-self.k:col+self.k+1]
        ...
        self.num_patches = (upper_row - self.k) * (upper_col - self.k)
    ...
    def calc_errors(self, up, left):
        ...
        if self.errors is None:
            self.errors = np.zeros((self.num_patches, self.w*2, self.overlap))
        ...
        for idx in range(self.candidates.shape[0]):
            ...
            if up is not None:
                self.errors[idx, :self.w, :] = meb.overlap_error(np.rot90(up),
                                                                 np.rot90(patch),
                                                                 self.overlap)
            if left is not None:
                self.errors[idx, self.w:, :] = meb.overlap_error(left, patch, self.overlap)

    def good_match(self, correspondence_patch, alpha=0.5):
        """
        ...
        tot_errors = np.sum(self.errors, axis=(1, 2))
        ...
        match_values = np.where(tot_errors <= np.min(tot_errors) * (1 + self.min_err))
        match_index = match_values[0][random.randint(0, match_values[0].shape[0] - 1)]

        return self.candidates[match_index], self.errors[match_index]

    def fill_patch(self, coord):
        """
        ...
        if coord[1] > self.k:
            left_n = self.filled[coord[0] - self.k:coord[0] + self.k + 1,
                                 ...
                                 self.adj_wind + self.k + 1]
        if self.correspondence is not None:
            correspondence_patch = self.correspondence[coord[0] - self.k:coord[0] + self.k + 1,
                                                       coord[1] - self.k:coord[1] + self.k + 1]

        # Scan source image and find a patch that satisfies error conditions.
        self.calc_errors(up_n, left_n)
        stitched, patch_errors = self.good_match(correspondence_patch)
    ...
    def fill_image(self):
        ...
        self.filled[:self.w, :self.w] = self.candidates[random.randint(0, self.num_patches)]

        # Fill each patch with an acceptable candidate.
        for row in range(self.k, self.dims[0] + 1, self.adj_wind):
            for col in range(self.k, self.dims[1] + 1, self.adj_wind):
                if row > self.k or col > self.k:
                    stitched = self.fill_patch((row, col))
